...
card_to_score = {card: index for (index, card) in enumerate(reversed(card_ordering))}


def load_input():
    puzzle_input = []
    with open('in07-2', 'r') as in_stream:
...
def get_hand_type(hand):
    ...
    jack_freq = 0
    if 'J' in freq \
            and len(freq) > 1:  # No need to substitute if it's already a five-of-a-kind
        jack_freq = freq.pop('J')
